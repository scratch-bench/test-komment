...
/**
* @description This function takes a number `a` as input and returns its ceiling 
* (the smallest integer greater than or equal to `a`).
* 
* @param { number } a - The `a` input parameter takes a number as input and passes 
* it to the `Math.ceil()` method to calculate and return the ceiling of that number.
* 
* @returns { number } Based on the documentation and the implementation of the 
* `ceiling` function provided above.
*/
...
/**
* @description This function takes a single argument 'a' which is a number and returns 
* its natural logarithm using the Math.log() method.
* 
* @param { number } a - The `a` input parameter passes the number to be logarithmized 
* to the `Math.log()` method.
* 
* @returns { number } The output returned by the `log` function is a number representing 
* the natural logarithm of the input `a`, which is also a number.
*/
